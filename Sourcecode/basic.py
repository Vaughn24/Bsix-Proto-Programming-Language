...
class BuiltInFunction(BaseFunction):
  ...
  def execute_abs(self, exec_ctx):
    ...
    number = int(text)
    if number < 0:
      return RTResult().success(Number(number*-1))
    elif number > 0:
      return RTResult().success(Number(number*1))

  execute_abs.arg_names = []

  def execute_help(self, exec_ctx):
    text = "===============================Syntax===============================\n"\
           "ABS()     Absolute value.\n" \
           "CLEAR()   Clears the terminal and all of its instances.\n" \
           "CNT()     String Count.\n" \
           "END       Stop the loop.\n" \
           "FOR       Used for iterating over a sequence.\n" \
           "FUN       Function declaration.\n" \
           "HELP()    Provides overview of commands available in the Terminal.\n" \
           "IF-ELSE   Conditional statements.\n" \
           "NOT       Returns False if the statement is True.\n" \
           "RETURN    Returns a command.\n" \
           "VAR       Variable, hold and store into memory.\n" \
           "WHILE     Executes a command as long as it is true."
    return RTResult().success(String(text))

  execute_help.arg_names = []


  def execute_countstr(self, exec_ctx):
    text = input()
    count = len(text)
    return RTResult().success(Number(count))
  execute_countstr.arg_names = []
...
BuiltInFunction.countstr = BuiltInFunction("countstr")
...
global_symbol_table.set("CNT", BuiltInFunction.countstr)
